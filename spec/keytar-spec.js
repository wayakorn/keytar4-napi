...
describe("keytar", function() {
  ...
  describe("setPassword/getPassword with targetname", function() {
    var targetname = service + '/' + account
    it("sets and get the password with targetname and credType specified", async function() {
      var credType = 1
      var credPersist = 1
      await keytar.setPassword(service, account, password, targetname, credType, credPersist)
      var actual = await keytar.getPassword(service, account)
      assert.isTrue(actual.length > 0)
    })
    it("sets and get the password with targetname specified but not credType", async function() {
      var targetname = service + '/' + account
      try {
        await keytar.setPassword(service, account, password, targetname)
        assert.fail('expect the invalid parameter exception to have occurred')
      } catch (err) {
        assert.isTrue(err.message.length > 10)
      }
    })
  })
